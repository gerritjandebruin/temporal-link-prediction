...
from .splitInIntervals import split_in_intervals
from .getInstances import get_instances
from .getTargets import get_targets
from .getBalancedSamples import get_balanced_samples
from .sampleDirect import sample_direct

from ..constants import CUTOFF, SAMPLE_SIZE, SPLIT_FRACTION

def from_edgelist_to_samples(
  path: str, *,
  ...
  """  
  index = path.split('/')[1]
  if index in [15, 17, 26, 27]:
    # Getting all instances will take too long. Subsample intelligently.
    print('Use intelligent subsampling.')
    return
  ...
  split_in_intervals(
    ...
    t_min=t_min, t_split=t_split, t_max=t_max, 
    verbose=verbose
  )
  ...
  # Step 4
  get_balanced_samples(path, sample_size=sample_size, verbose=verbose)